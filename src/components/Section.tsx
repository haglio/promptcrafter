    ...
    <section className={`section ${disabled ? 'disabled' : ''}`}>
      <div className="section-header">
        <div>
          ...
          <label className="section-label"><h3>{section.id}</h3></label>
        </div>
      ...
      {section.controls.map((control) => <Control key={control.id} control={control} state={state} actions={actions} parentWeighted={sectionWeight > 1} />)}
    </section>
  );
}
